			...
			form.ajaxSubmit({
				...
				success: function(data){
					if(typeof data.error != 'undefined' && data.error.length > 0){
						...
						//reset the choosen again.... because some of dropdowns where hidden...
                        //chosen destory is not working, because it was not binded perfectly due to invisible elements
                        var min_selections = jQuery('select[name=min_selections]');
                        min_selections.next('.chzn-container').remove();
                        min_selections.removeClass('chzn-done');
                        min_selections.prop('style', '');
                        min_selections.chosen(); //init the chosen again

                        var max_selections = jQuery('select[name=max_selections]');
                        max_selections.next('.chzn-container').remove();
                        max_selections.removeClass('chzn-done');
                        max_selections.prop('style', '');
                        max_selections.chosen(); //init the chosen again
                        //end
	...
	SurveyFactory.create_new_question = function(type, copy){
		...
		if(copy && jQuery('#qn-'+copy).length > 0){
			...
			if($('input[name="qtype"]').val() == 0 )
            {
                ...
                $('#message-modal').modal('show').find('.modal-body').html("<b>Only one question can create for each</b>");
            }
