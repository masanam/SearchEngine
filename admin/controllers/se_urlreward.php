...
class SearchEngineControllerse_urlreward extends JControllerLegacy
{
	...
	public function save(){
		...
		$model = JModelLegacy::getInstance('Se_urlreward', 'SearchEngineModel');
       	if( $model->save($data) ){
			...
			$this->setRedirect('index.php?option=com_searchengine&view=se_urlreward&task=se_urlreward.getlist&package_id='.$data->package_id, $msg);

		}else{

			$msg = 'Error in inserting url reward list. Please try agina ';
			$this->setRedirect('index.php?option=com_searchengine&view=se_urlreward&task=se_urlreward.edit&id='.$data->urlrewardid.'&package_id'.$data->package_id,$msg,'notice');
		}
	...
	public function delete(){
		...
		if ( $model->delete($cid) ){
			...
			$this->setRedirect('index.php?option=com_searchengine&view=se_urlreward&package_id='.JRequest::getVar('package_id'), 'Url reward list deleted successfully');		
		}else{
			$this->setRedirect('index.php?option=com_searchengine&view=se_urlreward&package_id='.JRequest::getVar('package_id'), 'Error on deleting. Please try again');		
		}		
	}
