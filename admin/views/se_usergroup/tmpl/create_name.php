...
<form action="index.php?option=com_searchengine&task=se_usergroup.save_usergroup&package_id=<?php echo $this->package_id; ?>" method="post" name="frm_save_name" id="frm_save_name" class="form-validate">
...
<table width="100%" cellpadding="1" cellspacing="1" class="table-striped"
	...
	<?php
	$rows = $this->model->filter_field($this->package_id,'name', $this->group->id);
	?>
	<tbody>
	<?php
	foreach ($rows as $row):
	?>
		<tr>
			...
			<td align="center">            
			<a href="<?php echo JRoute::_('index.php?option=com_searchengine&controller=se_usergroup&task=deletecriteria&field=name&package_id='.$this->package_id.'&criteria_id='.$row->criteria_id.'&usergroupid='.$row->usergroup_id); ?>" onclick="return window.confirm('Are you sure?');"><?php echo JText::_('COM_REFUND_DELETE'); ?></a>
