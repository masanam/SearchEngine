...
Joomla.submitbutton = function(pressbutton) {
	if(pressbutton=='se_usergroup.create'){
		window.location.href = "index.php?option=com_searchengine&controller=se_usergroup&task=create&package_id=<?php echo $this->package_id; ?>";
	}else{
...
<table align="center" border="0" class="table table-hover table-striped">
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<td align="center"><a href="<?php echo JRoute::_('index.php?option=com_searchengine&controller=se_usergroup&task=create&id='.$result->id.'&package_id='.$this->package_id); ?>"><?php echo $result->title==''? 'User Group '.$result->id:$result->title; ?>
		</a>
		</td>
