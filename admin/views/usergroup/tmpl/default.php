...
$filter_order = $app->getUserStateFromRequest('com_searchengine.usergroup.filter_order', 'filter_order', '', 'cmd');
...
function clickA(){
alert('You cannot add, edit or delete a user group setting when the search engine package is published.  \r\nAll user group settings must be completed BEFORE publishing the search engine package');
}

function save_edit(){
	jQuery('#task').val('save_usergroup');
	...
	function isNumberKey(evt)
	{
	 ...
	 function onSelectUserEmail(){
		...
		jQuery('form#adminForm').attr('action', 'index.php?option=com_searchengine&controller=usergroup');
		jQuery('form#adminForm').submit();	
	 }
	...
	<div class="container-fluid quiz-wrapper nospace-left no-space-left no-space-right">
		<div class="row-fluid">			
			...
			<div class="span6">
					...
					<tbody>
						<?php
							if (count( $this->registeredUser) > 0) {
								foreach ($this->registeredUser as $v1) {
									?>
							<tr>
					...
					<div style="float: right;"><?php echo $this->pagination->getListFooter();?></div>
				</form>
			</div>
		</div>
	</div>
