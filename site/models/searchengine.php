...
public function getHistory(){
  ...
  return $result;
    ...
    public function get_url($grupid){
      $this->_db = &JFactory::getDBO();
      $query = "SELECT * FROM `#__se_urlgrouplist_urls` WHERE urlgroup_id =$grupid " ;
      //SELECT * FROM `e3xlq_se_urlgrouplist_urls` WHERE `urlgroup_id` ='1'
      $this->_db->setQuery ( $query );
      $result = $this->_db->loadObjectList();
      return $result;
      	}

    function savegiftcode($user_id, $category_id, $gcid,  $type=1){
  		$db 	= JFactory::getDbo();
  		$user	= JFactory::getUser();
  		$now = JFactory::getDate();
  			$query = "insert into #__gc_recieve_user (category_id, user_id, gcid, date_time, status, giftcode_type)
  				values ('".$category_id."', '".$user_id."', '".$gcid."','".$now."', '0', '".$type."')";
  			$db->setQuery($query);
  			$db->query();
  		}
...
UNION ALL
 ...
 select a.id, a.title, 'Survey' as gctype,a.package_id,  (select sum(e.complete_giftcode_quantity+e.incomplete_giftcode_quantity) from  #__survey_question_giftcode as e
...
select 	a.id, a.title, 'Quiz' as gctype,a.package_id,  (select sum(e.complete_giftcode_quantity+e.incomplete_giftcode_quantity) from  #__quiz_question_giftcode as e
...
return $this->_db->loadObjectList();
  ...
  public function SerewardList($search_keyword,$packageId,$userid){
    $this->_db = &JFactory::getDBO();
    $query = "(select *, c.title as titlereward, f.title as titlesurvey, g.title as titleurl, h.title as titlequiz from  #__se_keywordgrouplist_keywords a
    LEFT JOIN #__se_keywordgrouplist b ON b.id = a.keywordgroup_id
    LEFT JOIN #__se_rewardlist c ON c.keywordgroup = b.id
    LEFT JOIN #__se_urlrewardlist e ON e.id = c.urlgroup
    LEFT JOIN #__se_urlrewardlist_settings_gc d ON d.urlrewardid = e.id
    LEFT JOIN #__se_surveygrouplist f ON f.id = c.surveygroup
    LEFT JOIN #__se_urlgrouplist g ON g.id = c.urlgroup
    LEFT JOIN #__se_quizgrouplist h ON h.id = c.quizgroup
    WHERE a.title='$search_keyword') ";
    $this->_db->setQuery ( $query );
    $result = $this->_db->loadObjectList();
    return $result;
}

public function UrlrewardList($urlgrup){
  $this->_db = &JFactory::getDBO();
  $query = "(select * from  #__se_urlrewardlist_settings_gc d
  LEFT JOIN #__se_urlrewardlist e ON e.id = d.urlrewardid
  LEFT JOIN #__se_rewardlist f ON f.urlgroup = e.id
  WHERE c.urlgroup ='$urlgrup') ";
  $this->_db->setQuery ( $query );
  $result = $this->_db->loadObjectList();
  return $result;
}

  public function surveycount($options = array(),$packageId,$userid){
    $this->_db = &JFactory::getDBO();
    $query = "(select * from  #__se_surveygrouplist_surveys
    LEFT JOIN #__survey s on s.id = #__se_surveygrouplist_surveys.survey_id
    where surveygroup_id=(select surveygroup from #__se_rewardlist where id=1)
    AND s.published=1) ";
    $this->_db->setQuery ( $query );
    $result = $this->_db->loadObjectList();
    return $result;
...
public function quizcount($options = array(),$packageId,$userid){
  $this->_db = &JFactory::getDBO();
  $query = "(select * from  #__se_quizgrouplist_quizs
  LEFT JOIN #__quiz_quizzes q on q.id = #__se_quizgrouplist_quizs.quiz_id
  where quizgroup_id=(select quizgroup from #__se_rewardlist where id=1)
  AND q.published=1)  ";
  $this->_db->setQuery ( $query );
  $result = $this->_db->loadObjectList();
  return $result;
...
public function urlgroupcount($options = array(),$packageId,$userid){
  $this->_db = &JFactory::getDBO();
  $query = "(select * from #__se_urlrewardlist_settings_ugs
  LEFT JOIN #__se_urlgrouplist ON #__se_urlgrouplist.id=#__se_urlrewardlist_settings_ugs.urllist
  WHERE urlrewardid  = (select urlgroup from #__se_rewardlist where id=1)
  AND #__se_urlgrouplist.published=1)  ";
  $this->_db->setQuery ( $query );
  $result = $this->_db->loadObjectList();
  return $result;
}
...
return $return;
        ...
        public function search($package_id){
	...
	function getUserSearchListCountDetail() {
		$db    = JFactory::getDBO();
		$query = $db->getQuery(true);
		$query = 'SELECT COUNT(*) FROM #__se_search_result_detail';
