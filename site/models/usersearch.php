...
class SearchEngineModelUsersearch extends JModelAdmin {
    ...
    public function savepenaltyform($data){
        $query = "INSERT INTO `#__penalties` (title,message,days,created_at) 
    ...
    public function getSPenaltyFormDetails($package_id,$user_id){
        $query = "SELECT a.created_at AS start_date , a.status, a.user_id, p.* FROM #__user_penalty AS a
                    LEFT JOIN #__penalties p ON p.id= a.penalty_form_id 
    ...
    public function updatepenaltyform($data){
        ...
        $query = "SELECT * FROM `#__penalties` WHERE assign_to = '$data->user_id' AND id ='$data->id' ";        
        $this->_db->setQuery($query);
        $result = $this->_db->loadObject();
        $insertid = '';

        if( $result ){
            $query = "UPDATE  `#__penalties` 
            ...
            $insertid = '';
             ...
             $this->_db->query();
              ...
              $query = "UPDATE  `#__user_penalty_history` 
                    SET penalty_form_id='$insertid'          
                    WHERE user_id='$data->user_id'
                    AND package_id='$data->package_id'";
             $this->_db->setQuery($query);
    ...
    public function getCreatedDate($data,$insertId=null){
        ...
        $query = "SELECT created_at FROM `#__user_penalty` WHERE penalty_form_id ='$ID' AND user_id= '$data->user_id'";        
        $this->_db->setQuery($query);
        $result = $this->_db->loadObject();
        return $result;
    ...
    public function addPenaltyToUser($uid, $pid, $package_id, $created_at){
        ...
        foreach ($uid as $key => $user_id) {               
            ...
            $result = $this->_db->loadObject();
            if( $result ){
                    $query = "  UPDATE `#__user_penalty` SET                        
        ...
        $query = "INSERT INTO `#__user_penalty_history` (user_id,package_id,created_at,penalty_form_id) VALUES " .implode(',',$history_values);       
        ...
        if(count($values)>0 ) {
            $query = "INSERT INTO `#__user_penalty` (user_id,package_id,created_at,penalty_form_id,status) VALUES " .implode(',',$values);            
            $db = JFactory::getDBO();
            $db->setQuery($query);
            $db->query();
        }
    ...
    public function removePenaltyToUser($uid, $package_id){
        ...
        foreach ($uid as $key => $user_id) {               
                ...
                $query = "  UPDATE `#__user_penalty` SET status= '0'"; 
                $query = $query." WHERE user_id= '$user_id' AND package_id='$package_id'";  
                $this->_db->setQuery($query);
                $this->_db->query();             
        }
    ...
    public function insert_criteria($data,$package_id) {
        ...
        switch($field){
            case 'name':
             $query = "INSERT INTO `#__usersearch_usergroup` (package_id,population,firstname,lastname,field) 
             VALUES ('$package_id','$data->population','$data->firstname','$data->lastname','$data->field')";
            break;           
            case 'email':
             $query = "INSERT INTO `#__usersearch_usergroup` (package_id,population,email,field) 
             VALUES ('$package_id','$data->population','$data->email','$data->field')";
            break;
            case 'age':
             $query = "INSERT INTO `#__usersearch_usergroup` (package_id,population,from_age,to_age,field) 
             VALUES ('$package_id','$data->population','$data->from_age','$data->to_age','$data->field')";
            break;
            case 'gender':
             $query = "INSERT INTO `#__usersearch_usergroup` (package_id,population,gender,field) 
             VALUES ('$package_id','$data->population','$data->gender','$data->field')";
            break;
            case 'location':
            $query = "INSERT INTO `#__usersearch_usergroup` (package_id,population,street,city,state,post_code,country,field) 
    ...
    public function update_criteria($data,$package_id,$criteria_id) {
        ...
        switch($field){
            case 'name':
             $query = "  UPDATE `#__usersearch_usergroup` SET 
    ...
    public function filter_field($package_id, $field) {
        $query = $this->_db->getQuery(true);
        $query->select('*');
        $query->from($this->_db->quoteName('#__usersearch_usergroup'));
        $query->where($this->_db->quoteName('package_id') . "='" . $package_id . "'");
    ...
    public function delete($criteria_id,$package_id){
        $query = "  DELETE FROM  `#__usersearch_usergroup` WHERE criteria_id= '$criteria_id' AND package_id='$package_id'";  
        $this->_db->setQuery($query); 
        $this->_db->query(); 
    ...
    public function serach($package_id, $serachfor = null ){
        ...
        $conditions2 = array();
        $conditions3 = array();
        $query = "SELECT *, CONCAT(`firstname`, ' ', `lastname`) AS NAME FROM #__ap_useraccounts";
        ...
        if(isset($criteria)){
           ...
           if( $serached_user ){ 
                ...
                $query = "SELECT u.*, b.`registerDate`, b.`username`,up.`status`,p.`days`,up.`created_at`, CONCAT(u.`firstname`, ' ', u.`lastname`) as fullname,
                          (SELECT count(*) FROM #__user_penalty_history WHERE user_id=u.id AND package_id=up.package_id) AS hcount,
                          (select count(*) as count from #__gc_recieve_user where user_id = u.id) AS symbol_queue,
                          (select count(*) as count from #__member_assigned_tickets where user_id = u.id) AS tcount 
                          FROM #__ap_useraccounts u
                          LEFT JOIN #__users b ON b.id= u.id
                          LEFT JOIN #__user_penalty up ON up.user_id= u.id 
                          LEFT JOIN #__penalties p ON p.id= up.penalty_form_id ";
    ...
    public function getHistory($user_id,$package_id){
        $query = "SELECT u.*, p.days,CONCAT(ua.firstname,' ', ua.lastname) AS fullname  from #__user_penalty_history u
                  LEFT JOIN #__penalties p ON p.id= u.penalty_form_id
                  LEFT JOIN #__ap_useraccounts ua ON ua.id= u.user_id
    ...
    public function getTicketHistory($user_id,$package_id){
        ...
        $query->join( 'INNER',$db->QuoteName('#__ap_useraccounts','ua'). ' ON (' . $db->quoteName('ua.id') . ' = ' . $db->quoteName('a.user_id') . ')' );
