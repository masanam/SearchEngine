...
Joomla.submitbutton = function(pressbutton) {
	...
	{		
			window.location.href = "index.php?option=com_searchengine&views=se_urlreward";	
	}	
	else{
...
<style>
#additionalsettings{width:100px;}
.margin-top-7{margin-top:7px;}
</style>
<div style="clear:both;"></div>
<?php echo $this->loadTemplate('nav');?>
<div id="j-main-container" class="span12">
<form method="post" action="<?php echo JRoute::_('index.php?option=com_searchengine&controller=se_urlreward&task=se_urlreward.save');?>" name="adminForm" id="adminForm">
	...
	<div class="navbar-inner" style="background-color: #fafafa;">	
		<div class="header-container">
			<div class="cq-nav-collapse nav-collapse collapse">
				<ul class="nav">					
					...
					<li>
						<a href="<?php echo JRoute::_('index.php?option=com_searchengine&view=se_urlreward');?>">
		...
		<div>
				...
				<div class="span10">
					...
					<div class="span10">
						<input type="text" name="title" id="title" data-rule-required="true" data-msg-required="This field is required"s>
					</div>
				</div><br><br>
			<div class="span12" id="gift_code_rule_list_week_days">
				<div style="float:left;width:100%;border:1px solid #CCCCCC;margin-bottom: 25px;" id="group1" class="bigbox">

				    <div class="span12" style="width:100%;">
				    	
				    	<div style="background-color:#CCCCCC;height: 50px;">
						
					    	<div class="span4 margin-top-7" style="text-align:center;"><strong><?php echo JText::_('Url clicked (No url needed to be clicked if left blank)');?></strong></div>
					    	<div class="span5 margin-top-7" style="text-align:center;"><strong><?php echo JText::_('Giftcode');?></strong></div>
						<div class="span3 margin-top-7" style="text-align:center;"><strong><?php echo JText::_('Additional Settings');?></strong></div>
				    	</div>
				    </div>
					<div class="span12">
<div class="span4 divtableurllist">
							<table align="center" border="0" class="table table-striped"  style="width:97%;">
								<thead>									
									<tr class="actions">
										...
										<td colspan="3" align="right"><a href="javascript:void(0)" class="deleteAll">Delete</a><a href="#" data-toggle="modal" data-target="#myModalWeekdays2" data-group-id="1">Add</a></td>
									</tr>
								</thead>
							</table>
							<table align="center" border="0" class="table table-striped" style="width:97%;">
						...
						<div class="span5 divtablecategory" style="border-right: 1px solid #CCCCCC; border-left: 1px solid #CCCCCC; padding:0 15px;">
							<table align="center" border="0" class="table table-striped" width="70%">
								<thead>									
									<tr class="actions">
										<input type="hidden" name="group-id[]" value="1">
										<td colspan="3" align="right"><a href="javascript:void(0)" class="deleteAll">Delete</a><a href="#" data-toggle="modal" data-target="#myModalWeekdays" data-group-id="1">Add</a></td>
									</tr>
								</thead>
							</table>
						...
						<input type="textbox" id="additionalsettings" name="additionalsettings" value="" /> Times per day
						</div></div>
					</div>
				...
				<div class="modal-dialog modal-sm">
				  <div class="modal-content">
					<div class="modal-header">
					  ...
					  <h4 class="modal-title">Add Url List</h4>
					</div>
	...
	jQuery(document).ready(function(){
		...
		jQuery(document).on("click","#save-day",function(){
			...
			if (selected.length > 0) {
				...
				var boxid = jQuery(this).parent().find("input").val();
				jQuery("#group"+boxid).find(".divtablecategory table tbody").append("<tr><td><input type='checkbox' name='ruleweekday[]' class='chkWeekDay"+boxid+"' value='"+selectedVal+"'><input type='hidden' name='weekdays["+boxid+"][day][]' value='"+selectedVal+"'></td><td>"+selectedVal+"</td></tr>");
				jQuery('#myModalWeekdays').modal('hide') 
			}
			else{
			...
			if (selected.length > 0) {
				...
				jQuery(selected).each(function(){
					...
					if( selectedVal!=''){
						...
						jQuery("#group"+boxid).find(".divtableurllist table tbody").append("<tr><td><input type='checkbox' name='ruleticketid[]' class='chkDayMemberTicket"+boxid+"' value='"+selectedVal[0]+"'><input type='hidden' name='weekdays["+boxid+"][ticket][]' value='"+selectedVal[0]+","+selectedVal[1]+","+selectedVal[2]+"'></td><td>"+selectedVal[1]+"</td></tr>");
					}
				});
				...
				alert("Please select any url group list.")
			}
		});
