<form action="index.php?option=com_searchengine&task=se_usergroup.save_usergroup" method="post" name="frm_save_gender" id="frm_save_gender" class="form-validate">
...
<table width="100%" cellpadding="1" cellspacing="1"
	...
	<?php
	$rows = $this->model->filter_field($this->packageId,'gender',$this->group->id);
	?>
	<tbody>
	<?php
	foreach ($rows as $row):
	?>
		<tr>
			...
			<a href="<?php echo JRoute::_('index.php?option=com_searchengine&controller=se_usergroup&task=deletecriteria&field=gender&criteria_id='.$row->criteria_id.'&usergroupid='.$row->usergroup_id); ?>" onclick="return window.confirm('Are you sure?');"><?php echo JText::_('COM_REFUND_DELETE'); ?></a>
			</td>
		</tr>
		<?php
		endforeach;
