...
if($this->userId>0 && $this->packageId > 0){ ?>
  &nbsp;&nbsp;&nbsp;<button class="btn btn-primary" onclick="submitForm('<?php echo JRoute::_('index.php?option=com_searchengine&reward=0&keyword='.$keyword.''); ?>')">Search</button>
  <div class="pull-right">
  <a class="btn" href="<?php echo JRoute::_('index.php?option=com_searchengine&view=dashboard');?>">Search engine rewards</a>

</div>
<?php } else { ?>
  &nbsp;&nbsp;&nbsp;<button class="btn btn-primary" onclick="submitForm('<?php echo JRoute::_('index.php?option=com_searchengine&reward=1&keyword='.$keyword.''); ?>')">Search</button>
...
//echo "default <pre>";print_r($this->search_results);
	 if( $this->searchData ){
		...
		if ($this->searchData['type'] === 'image'){
			foreach ($this->search_results as $search_result){
				echo '<a href="'.$search_result['link'].'">'.'<img src="'.$search_result['img_src'].'" ></a>'."<br /><br />";
			}
		}
